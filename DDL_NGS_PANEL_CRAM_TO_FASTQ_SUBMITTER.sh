...
# INPUT ARGUMENTS
	...
	QUEUE_LIST=$4 # optional. if no 2nd argument present then the default is cgc.q
	...
	REF_GENOME=$6 # optional. if no 5th argument present then assumes grch37. full path
...
# for each sample in sample sheet convert cram file to fastq and build the hold id so that an email can be sent after all fastq after all fastq files have finished being created.
	...
	for SM_TAG in $(zcat ${SAMPLE_SHEET} \
		| awk 1 \
		| sed 's/\r//g; /^$/d; /^[[:space:]]*$/d' \
		| awk 'BEGIN {FS=","} \
			NR>1 \
...
# SEND EMAIL SAYING THAT CONVERSION WAS FINISHED.
	...
	END_EMAIL_WHEN_DONE ()
	{
		...
		qsub \
			${QSUB_ARGS} \
		-N DDL_NGS_PANEL_CRAM_TO_FASTQ_FINISH_EMAIL_${SGE_SM_TAG}_${PROJECT_NAME} \
